...
onMounted(() => {
    document.addEventListener('mousedown', (e) => {
        let containers = document.getElementsByClassName('language-select-menu');
        let max = containers.length;
        let count = 0;
...
<template>
    <nav class="navbar">
        <div class="items" :class="{shift: !previousRoute}">
            <EffectButton class="back-arrow" :to="previousRoute"><ArrowLeft /></EffectButton>
            <h1>IR<span>_</span></h1>
        </div>
        <div class="spacer" />
        <EffectButton @click="toggleColorMode" :colour="themeIconColour">
            <HalfMoon v-if="colorMode.value === 'light'"/>
            <SunLight v-if="colorMode.value === 'dark'"/>
        </EffectButton>
        <div class="spacer-mini" />
        <EffectButton @click="languageSelector = !languageSelector" :disabled="languageSelector"><Globe /></EffectButton>
        <LanguageSelect :open="languageSelector" @close="languageSelector = false"/>
    </nav>
</template>
