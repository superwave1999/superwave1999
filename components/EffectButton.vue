...
const cssVarColourHover = computed(() => {
    return props.colour ? `var(${props.colour}-border-hover)` : 'var(--accent-border-hover)';
});
const cssVarColourActive = computed(() => {
    return props.colour ? `var(${props.colour}-border-active)` : 'var(--accent-border-active)';
});
...
function shadowEffect(e: MouseEvent) {
    ...
    const link = (<HTMLElement> e.target).closest('a.link');
    if (!link) {
        return;
    }
    ...
    (<HTMLElement> linkElement).style.cssText = `left: ${l}px; top: ${t}px; filter: blur(${blur}px); color: ${cssVarColour.value}`;
}
</script>
...
<template>
    <NuxtLink
        ...
        :href="href ? href : null"
        class="link"
        :class="{mini, text: Boolean(text), 'icon-right': iconRight, noclick: disabled}"
    >
        <svg
            class="decoration"
        ...
        >
		<path fill="currentColor" d="M10 500a490 490 0 1 0 980 0 490 490 0 0 0-980 0z" />
            <path
                fill="currentColor"
