...
defineI18nRoute({
    ...
    const page = await queryContent(section)
        .where({ _path: path })
        .findOne()
...
const hasAnyMeta =
    ...
    onMounted(() => {
        nextTick(() => { //Wait for child (nuxt-content) to populate
...
<template>
    <article>
        ...
        <ContentRenderer :value="page" class="nuxt-content" id="nuxt-content"/>
        <Divider v-if="hasAnyMeta"/>
        <div v-if="hasAnyMeta" class="post-meta">
            <div v-if="page.infoPlatform">
                <h4>Platform:
                    <Separator />{{page.infoPlatform}}
                </h4>
            </div>
            <div v-if="page.infoStack" class="stack-box">
                <h4>
                    Stack:
                    <Separator />{{page.infoStack}}
                </h4>
            </div>
            ...
            <div v-if="page.links && page.links.length" class="link-box">
                <h4 v-for="link of page.links">
                    <a :href="link.href" target="_blank"><Link />&nbsp;{{link.title}}</a>
                </h4>
            </div>
        </div>
        <!-- TODO: Prev/next -->
